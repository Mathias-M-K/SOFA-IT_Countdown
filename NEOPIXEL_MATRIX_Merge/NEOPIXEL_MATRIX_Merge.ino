...
void loop() {
  ...
  Serial.print("Next Date: ");
  printDate(getEarliestDate());

  Serial.print("Time to next date: ");
  Serial.println(howLongTilDate(getEarliestDate()));

  String stringToPrint = howLongTilDate(getEarliestDate());


  if(stringToPrint.length() > 2){
    writeLongString(stringToPrint);
  }else{
    writeString(stringToPrint,0,1);
  }

  if(getTimeToDate(getEarliestDate())<60){
    delay(500);
  }else{
    delay(5000);
  }
...
time_t getEarliestDate(){
  ...
  for (int i = 0; i <= nrOfDates; i++) {
    double timeNow = timeClient.getEpochTime();
    double date = dates[i];
